...
createSpecificBtn(hypoBtn, "Calculate Mark");
...
function calculateHypotheticalMark() {
  let course = currentCourse.name, courseMark = 0;
  //let i = getObjIndex (currentCourse.name, courseArr);
  //if (m != -1) {
    for (let m = 0; m < currentCourse.assignmentArr.length; m++) {
      if (currentCourse.assignmentArr[m].done == false) {
        //the month line below has some wierd error
        currentCourse.assignmentArr[m].hypothetical_mark = window.prompt("What is the mark you expect to get in the assignment " + currentCourse.assignmentArr[m].name + "?");
        ...
        courseMark = courseMark + ((currentCourse.assignmentArr[m]).hypothetical_mark) * currentCourse.assignmentArr[m].weight;
    } else {
      courseMark = courseMark + ((currentCourse.assignmentArr[m]).mark) * currentCourse.assignmentArr[m].weight;
    }
    //}
  }
  alert("Your hypothetical mark for " + course + " is " + courseMark + "%.");
  return;
}
...
function calculateNeededAssignmentMark() {
  let assignName, assignIndex = -1, goalMark = 0;
  let total = 0, sum = 0, neededMark = 0;
  assignName = window.prompt("What assignment would you like calculate for?");
  assignIndex = getObjIndex(assignName, currentCourse.assignmentArr);
  if (assignIndex == -1) {
    alert("Assignment name not found.");
    return;
  }
  if ((currentCourse.assignmentArr[assignIndex]).done == true) {
    alert("You have completed this assignment.");
    return;
  }
  let assignment = currentCourse.assignmentArr[assignIndex];
  ...
  for (let i = 0; i < (currentCourse.assignmentArr).length; i++) {
    if ((currentCourse.assignmentArr[i]).done == true) {
      let currentAssignment = currentCourse.assignmentArr[i];
      total = total + currentAssignment.weight;
      sum = sum + ((currentAssignment.weight) * (currentAssignment.mark));
    }
  }
  neededMark = ((goalMark - sum)/total) / ((assignment.weight) / total);
